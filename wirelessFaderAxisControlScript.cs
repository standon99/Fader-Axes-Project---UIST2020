...
// any changes to variables must be passed as function inputs, or manipulated within the axis class itself
{
    ...
    public bool turnOffAllHaptics = false, discretizeAxis = false, followMode = false, runHapticsDemoMode = false, liveHapticsMode = true, joystickMode = false;
    ...
    public int[] jsModeOneReceived;
    public int[] jsModeTwoReceived;
    ...
    int count;
    public int itr = 0;//, rightPos = 3222;
    ...
    public class Axis
    {
        ...
        private bool messg_rcv_stat = false;
        public int[] jsModeOne;
        public int[] jsModeTwo;
        ...
        public void discreteAxes(SerialPort serialPort, int hapticsVal1, int hapticsVal2, int xSteps, int ySteps, int zSteps, int value1, int value2)
        {
            //WirelessArduinoSlidesAndRotary.ArduinoReaderHaptics asar;
            // asar = new WirelessArduinoSlidesAndRotary.ArduinoReaderHaptics();
            //  Wireless_axes wa = new Wireless_axes();
            ...
            wirelessFaderAxisControlScript ba = new wirelessFaderAxisControlScript();
            string sentMsg = ba.messageCreator(0, 0, 2, xSteps, ySteps, zSteps, hapticsVal1, hapticsVal2);
            try
            {
                serialPort.WriteLine(sentMsg);
            }
        ...
        public void toggleFollowMode(SerialPort serialPort, int dist, int hapticsVal1, int hapticsVal2, int val2)
        {
            int hapticsStat = 5;
            if (dist != 0)
            {
                ...
                wirelessFaderAxisControlScript ba = new wirelessFaderAxisControlScript();
                string sentMsg = ba.messageCreator(dist, val2, hapticsStat, 0, 0, 0, hapticsVal1, hapticsVal2);
                try
                {
                    serialPort.WriteLine(sentMsg);
                }
        ...
        public void toggleJoyStickMode(SerialPort serialPort)
        {
            int hapticsStat = 6;
            wirelessFaderAxisControlScript ba = new wirelessFaderAxisControlScript();
            string sentMsg = ba.messageCreator(0, 0, hapticsStat, 0, 0, 0, 0, 0);
            try
            {
                serialPort.WriteLine(sentMsg);
            }
        ...
        // Method reads the ESP's messages detailing button press values and slider positions
        public void readAxis(int axis_num, string serialReadLine, int no_axes, bool jsModeStat)
        {
            wirelessFaderAxisControlScript ba = new wirelessFaderAxisControlScript();
            ...
            if (firstReceiveFlag == 0)
            {
                ...
                firstReceiveFlag = 1;
                jsModeOne = new int[no_axes];
                jsModeTwo = new int[no_axes];
            }
            ...
            {
                jsModeTwo[axis_num] = Int32.Parse(numbers[1]);
                jsModeOne[axis_num] = Int32.Parse(numbers[0]);
            }
        }
    ...
    void Update()
    {
        if (itr == 0)
        {
            ...
            jsModeOneReceived = new int[noAddress];
            jsModeTwoReceived = new int[noAddress];
        }
        //print("NUMADD = " + numAdd);
        ...
        for (int j = 0; j < numAdd; j++)
        {
            ...
            {
                axisClass.readAxis(j, readSerial, numAdd, joystickMode);
                ...
                if (joystickMode)
                {
                    jsModeOneReceived[j] = axisClass.jsModeOne[j];
                    jsModeTwoReceived[j] = axisClass.jsModeTwo[j];
                }
            }
            else if (joystickMode)
            {
                jsModeOneReceived[j] = 0;
                jsModeTwoReceived[j] = 0;
            }
            ...
            else if (liveHapticsMode)
            {
                ...
                if (Math.Abs(rightPositionsReceived[j] - 600) <= 80)
                {
                    hapticsVal1 = 2;
                }
                else
            ...
            if (!followMode && !discretizeAxis & !joystickMode)
            {
            ...
            else if (discretizeAxis) axisClass.discreteAxes(sp[j], hapticsVal1, hapticsVal2, xSteps, ySteps, zSteps, leftPos, rightPos);
            else if (followMode) axisClass.toggleFollowMode(sp[j], followDist, hapticsVal1, hapticsVal2, followModePos);
            else if (joystickMode)
            {
                runHapticsDemoMode = false;
                liveHapticsMode = false;
                turnOffAllHaptics = true;

                axisClass.toggleJoyStickMode(sp[j]);
            }
